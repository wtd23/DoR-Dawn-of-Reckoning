...
namespace Common
{
    ...
    [Serializable]
    public class characters_mails : DataObject
    {
        private int _Guid;
        ...
        public string ItemsString
        {
            ...
            {
                string Value = "";
                foreach (mail_item Item in Items)
                {
            ...
            set
            {
                ...
                foreach (string Obj in Objs)
                {
                    ...
                    if (ItemInfo.Length == 2)
                        Items.Add(new mail_item(uint.Parse(ItemInfo[0]), null, ushort.Parse(ItemInfo[1])));
                    else
                        Items.Add(new mail_item(uint.Parse(ItemInfo[0]), ItemInfo[1], ushort.Parse(ItemInfo[2])));
                }
                Dirty = true;
            }
        }

        public List<mail_item> Items = new List<mail_item>();
    }
}